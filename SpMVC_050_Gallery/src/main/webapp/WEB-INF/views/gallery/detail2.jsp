...
<div id="gallery_box">
	<div>
		<c:if test="${empty GALLERY.g_image}">
			<img src="${rootPath}/files/noimage.png" alt="main_iamge" width="200px"/>
		</c:if>
		<c:if test="${not empty GALLERY.g_image}">
			<img src="${rootPath}/files/${GALLERY.g_image}" alt="main_iamge" />
		</c:if>
	</div>
	<div id="gallery_info">
		<h3>제목 : ${GALLERY.g_subject}</h3>
		<h5>SEQ : ${GALLERY.g_seq}</h5>
		<p>작성일자 : ${GALLERY.g_date}</p> 
...
<script>
let update_btn = document.querySelector("button.gallery.update");
let delete_btn = document.querySelector("button.gallery.delete");
...
update_btn.addEventListener("click",()=>{
	...
	/*
		...
		현재 작성한 코드는 JSP 코드이다
		이 코드는 Response가 될 때 HTML코드로 변환이 되고
		script부분도 변환이 되는데
...
/*
 	...
 	동시성처리(멀티 환경에서 서로 변수가 간섭하는 현상을 핸들링)를 쉽게 하기 위한
 	방안으로 사용한다
