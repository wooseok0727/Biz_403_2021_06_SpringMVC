...
public class MyController {
	 ...
	 * 		절대 허용하지 않는다
	 * 
	 * RequestMapping이 같고 ,RequestMethod가 다르면
	 * 		같이 사용할 수 있다.
	 * 
	 ...
	 * 		 요청을 받았으면 
	 * 		 @RequestMapping(value=/insert",method=RequestMethod.GET)을 처리하는
	 * 		 method가 있어야 한다
	 * 
	 *  Mapping(vlaue="/insert) 가 있는데
	 *  method가 POST만 있을 경우 서버는 406 http 상태코드(Status Code)를
	 *  응답한다.
	 */
	 ...
	 * 변수를 받는다
	 * 이런 코드는 개발자가 신경쓰지 않아도 자등으로
	 * 작동되는 코드이다.
	 * getParameter() 등의 코드를 이제는 잊어도 된다
	 * 
	 * 그런데,
	 * Java method의 매개변수를 String형이 아닌 것으로
	 * 		(숫자형, Boolean 형 등)
